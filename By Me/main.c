...
Movimiento busca_movimiento_por_id(Lista_movimiento m,int id){                   //ITEM B : "Busca un movimiento por id, no figura en menu y tampoco muestra"
    reset_lista_movimiento(&m);
    while(!isOos_lista_movimiento(m)){
        if (m.cur.vipd.id_mov == id){
            get_id_mov(m);
        }
        else{
         forward_lista_movimiento(m);
